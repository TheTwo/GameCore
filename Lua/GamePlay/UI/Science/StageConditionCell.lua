...
local StageConditionCell = class('StageConditionCell',BaseTableViewProCell)

function StageConditionCell:OnCreate(param)
...
function StageConditionCell:OnFeedData(stageId) --不能传相同table进去，所以传进来的是stageId*10000
...
function StageConditionCell:OnClickNewChapter()
...
function StageConditionCell:OnBtnGoto1Clicked(args)
    --local stageConfig = ConfigRefer.CityTechStage:Find(self.stageId)
    --local unlockBuilding = stageConfig:UnlockBuilding()
    --todo 跳转建筑？
end
function StageConditionCell:OnBtnGoto2Clicked(args)
    --todo 引导去加科技点？
end
function StageConditionCell:OnBtnGoto3Clicked(args)
    local stageConfig = ConfigRefer.CityTechStage:Find(self.stageId)
    local lastTech = stageConfig:LastTech()
    g_Game.EventManager:TriggerEvent(EventConst.ON_LOCK_TO_TECH, ConfigRefer.CityTechLevels:Find(lastTech):Type())
end

return StageConditionCell
