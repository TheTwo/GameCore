...
namespace CSObjectWrapEditor
{
    ...
    public struct UserConfig
    {
        ...
        public IEnumerable<Type> AdditionalLinkXmlTypes;
    }
    ...
    public static class Generator
    {
        ...
        static bool isMemberInBlackList(MemberInfo mb)
        {
            ...
            if (mb is FieldInfo { FieldType: { IsPointer: true } }) return true;
            if (mb is PropertyInfo propertyInfo && (propertyInfo.PropertyType.IsByRefLike || propertyInfo.PropertyType.IsPointer)) return true;
        ...
        static bool CheckIsByRefLikeSmart(MethodBase mb, ParameterInfo[] parameters)
        {
	        var checkList = new List<(ParameterInfo, Type)>();
	        foreach (var parameterInfo in parameters)
	        {
		        var parameterType = parameterInfo.ParameterType;
		        if (parameterType.IsByRefLike)
		        {
			        checkList.Add((parameterInfo, parameterType));
		        }else if (parameterType.IsByRef && parameterType.HasElementType &&
		                  parameterType.GetElementType()!.IsByRefLike)
		        {
			        checkList.Add((parameterInfo, parameterType.GetElementType()));
		        }
	        }
	        if (checkList.Count <= 0) return false;
	        if (checkList.Any(pInfo => pInfo.Item2.IsByRefLike && (!ParameterRefLikeTypeWhiteList.Contains(pInfo.Item2) || pInfo.Item1.IsOut)))
	        {
		        return true;
	        }
	        var dType = mb.DeclaringType;
	        if (null == dType) return false;
	        if (mb.IsConstructor)
	        {
		        var sameNameMethod =
			        dType.GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.IgnoreCase);
		        return sameNameMethod.Count(m => string.CompareOrdinal(m.Name, mb.Name) == 0) > 1;
	        }
	        else
	        {
		        var sameNameMethod =
			        dType.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static);
		        return sameNameMethod.Count(m => string.CompareOrdinal(m.Name, mb.Name) == 0) > 1;
	        }
        }
        ...
        static bool isMethodInBlackList(MethodBase mb)
        {
            ...
            //指针目前不支持，先过滤
            if (parameters.Any(pInfo => pInfo.ParameterType.IsPointer)) return true;
            if (mb is MethodInfo { ReturnType: { IsPointer: true } }) return true;
            // RefLike 的栈上类型目前的结构也是支持不了的，也需要过滤
            if (CheckIsByRefLikeSmart(mb, parameters))
            {
	            return true;
            }
            if (parameters.Any(pInfo =>
	                pInfo.ParameterType.IsByRefLike && !ParameterRefLikeTypeWhiteList.Contains(pInfo.ParameterType)))
            {
	            return true;
            }
            if (mb is MethodInfo { ReturnType: { IsByRefLike: true } }) return true;
        ...
        public static HashSet<Type> ParameterRefLikeTypeWhiteList = null;

        public static HashSet<Type> AdditionalLinkXmlTypes = null;
...
#pragma warning restore 618
            ...
            if (isDefined(test, typeof(ParameterRefLikeTypesAttribute)) && typeof(IEnumerable<Type>).IsAssignableFrom(cfg_type))
            {
                ParameterRefLikeTypeWhiteList.UnionWith(get_cfg() as IEnumerable<Type>);
            }

            if (isDefined(test, typeof(AdditionalLinkXmlTypesAttribute)) &&
                typeof(IEnumerable<Type>).IsAssignableFrom(cfg_type))
            {
	            AdditionalLinkXmlTypes.UnionWith(get_cfg() as IEnumerable<Type>);
            }
...
#endif
            ParameterRefLikeTypeWhiteList = new HashSet<Type>();

            AdditionalLinkXmlTypes = new HashSet<Type>();
        ...
        [MenuItem("DragonReborn/XLua/Generate Code", false, 1)]
        ...
        [MenuItem("DragonReborn/XLua/Clear Generated Code", false, 2)]
...
#if UNITY_2018
        [MenuItem("DragonReborn/XLua/Generate Minimize Code", false, 3)]
        ...
        public static void CustomGen(string template_src, GetTasks get_tasks)
        {
            ...
            foreach (var gen_task in get_tasks(luaenv, new UserConfig() {
                ...
                AdditionalLinkXmlTypes = AdditionalLinkXmlTypes,
            }))
            {
                LuaTable meta = luaenv.NewTable();
                meta.Set("__index", luaenv.Global);
