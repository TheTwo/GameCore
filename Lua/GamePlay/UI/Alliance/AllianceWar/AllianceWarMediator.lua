...
local AllianceWarTabBattle = require("AllianceWarTabBattle")
...
---@class AllianceWarMediatorParameter
---@field enterTabIndex number
---@field backNoAni boolean

---@class AllianceWarMediator:BaseUIMediator
---@field new fun():AllianceWarMediator
---@field super BaseUIMediator
local AllianceWarMediator = class('AllianceWarMediator', BaseUIMediator)

function AllianceWarMediator:ctor()
    BaseUIMediator.ctor(self)
    self._selectedTab = nil
    ---@type AllianceWarTabBuildUp|AllianceWarTabBattle|AllianceWarTabBuilding
    self._selectedTabLogic = nil
    self._backNoAni = false
end

function AllianceWarMediator:OnCreate(param)
    ...
    self._p_tabBattle = AllianceWarTabBattle.new(self, "p_group_war")
    self._p_tabBuilding = AllianceWarTabBuilding.new(self, "p_group_building")
    self._p_tabBuildUp._p_root:SetVisible(false)
    self._p_tabBattle._p_root:SetVisible(false)
    self._p_tabBuilding._p_root:SetVisible(false)
end

---@param param AllianceWarMediatorParameter|nil
function AllianceWarMediator:OnOpened(param)
    ...
    tabData = {}
    tabData.index = 3
    tabData.btnName = I18N.Get("alliance_war_title3")
    tabData.isLocked = false
    tabData.onClick = clickFunc
...
function AllianceWarMediator:OnShow(param)
...
function AllianceWarMediator:OnHide(param)
...
function AllianceWarMediator:OnClickTabIndex(index)
...
function AllianceWarMediator:SetTabHasData(hasData)
    self._p_group_none:SetVisible(not hasData)
end

function AllianceWarMediator:SetupTrackNotify(add)
    ...
    local warNode = self._child_tab_left_btn_1:GetNotificationNode()
    local siegeNode = self._child_tab_left_btn_2:GetNotificationNode()
    ...
    if add then
        ...
        if warNode then
            ...
            notificationModule:AttachToGameObject(dyNode, warNode.go, warNode.redTextGo, warNode.redText)
        end
        if siegeNode then
        ...
        if warNode then
            notificationModule:RemoveFromGameObject(warNode.go, false)
        end
        if siegeNode then
...
function AllianceWarMediator:OnLeaveAlliance(allianceId)
    self:CloseSelf()
end

function AllianceWarMediator:OnClickBackBtn()
    self:BackToPrevious(nil, self._backNoAni, self._backNoAni)
end

return AllianceWarMediator