    ...
    public partial class StaticLuaCallbacks
    {
        ...
        internal static int LoadFromStreamingAssetsPath(RealStatePtr L)
        {
            try
            {
                ...
                if (File.Exists(filepath))
                {
                    ...
                    NLogger.Warn("load lua file from StreamingAssets is obsolete, filename:{0}", filename);
        ...
        internal static int LoadFromCustomLoaders(RealStatePtr L)
        {
            try
            {
                ...
                foreach (var customLoader2 in self.customLoaders2)
                {
	                string real_file_path = filename;
	                var bytes = customLoader2(ref real_file_path);
	                if (bytes.IsEmpty) continue;
	                unsafe
	                {
		                fixed (byte* pointer = bytes)
		                {
			                if (LuaAPI.xluaL_loadbuffer(L, new IntPtr(pointer), bytes.Length, "@" + real_file_path) != 0)
			                {
                ...
                foreach (var loader in self.customLoaders)
                {
                    ...
                    if (bytes != null)
                    {
                        ...
                        {
                            // EventManager.Instance.TriggerEvent(new LoadExistLuaFail(filename));
                            return LuaAPI.luaL_error(L, String.Format("error loading module {0} from CustomLoader, {1}",
                                LuaAPI.lua_tostring(L, 1), LuaAPI.lua_tostring(L, -1)));
                        }
                        return 1;
        ...
        [MonoPInvokeCallback(typeof(LuaCSFunction))]
        public static int NewTable(RealStatePtr L)
        {
            try
            {
                var naar = 0;
                ...
                if (top >= 1)
                {
                    naar = LuaAPI.xlua_tointeger(L, 1);
                }

                if (top >= 2)
                {
                    nrec = LuaAPI.xlua_tointeger(L, 2);
                }
                LuaAPI.lua_settop(L, 0);
                LuaAPI.lua_createtable(L, naar, nrec);
                return 1;
            }
            catch (LuaStackTraceException e)
            {
                UnityEngine.Debug.LogError(e);
                return LuaAPI.luaL_error(L, "c# exception in NewTable: " + e);
            }
            catch (Exception e)
            {
                return LuaAPI.luaL_error(L, "c# exception in NewTable: " + e);
