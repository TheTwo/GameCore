...
namespace XLua
{
    ...
    // 过滤 RefLikeType 作为参数的白名单
    public class ParameterRefLikeTypesAttribute : Attribute
    {
        
    }
    
    // 额外添加到link.xml 的类型
    public class AdditionalLinkXmlTypesAttribute : Attribute
    {
        ...
        static Dictionary<Type, List<string>> AdditionalProperties
        {
            get
            {
                return new Dictionary<Type, List<string>>()
                {
                    ...
                    { typeof(UnityEngine.Plane), new List<string>() { "normal", "distance"} },
                };
            }
        }
    }
