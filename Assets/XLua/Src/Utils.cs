...
namespace XLua
{
	...
	{
		private static readonly Func<MethodInfo, IReadOnlyList<ParameterInfo>> GetParametersWrapper;

		static Utils()
		{
			Func<MethodInfo, IReadOnlyList<ParameterInfo>> impl = null;
			try
			{
#if NET_STANDARD
				var t = Type.GetType("System.Reflection.RuntimeMethodInfo");
				var getParametersNoCopy = t?.GetMethod("GetParametersInternal",
					BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
				if (null != getParametersNoCopy)
				{
					impl = info =>
						t.IsInstanceOfType(info)
							? (ParameterInfo[])getParametersNoCopy.Invoke(info, Array.Empty<object>())
							: info.GetParameters();
				}
#else
				var getParametersNoCopy = typeof(MethodInfo).GetMethod("GetParametersNoCopy",
					BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
				if (null != getParametersNoCopy)
				{
					var getParametersNoCopyImpl =
						(Func<MethodInfo, ParameterInfo[]>)Delegate.CreateDelegate(
							typeof(Func<MethodInfo, ParameterInfo[]>), getParametersNoCopy);
					impl = info => getParametersNoCopyImpl(info);
				}
#endif
			}
			catch
			...
			impl ??= info => info.GetParameters();
			GetParametersWrapper = impl;
		}
		...
		public static List<Type> GetAllTypes(bool exclude_generic_definition = true)
		{
			...
			for (int i = 0; i < assemblies.Length; i++)
			{
				try
				{
					...
					{
#endif
						if (string.CompareOrdinal(assemblies[i].FullName, "MTE, Version=4.0.0.0, Culture=neutral, PublicKeyToken=null") == 0) continue;
		...
		static LuaCSFunction genFieldGetter(Type type, FieldInfo field)
		{
			if (field.IsStatic)
			{
				...
				{
#if  XLUA_PROFILING
					Profiler.BeginSample($"[xLua]{field.DeclaringType}.{field.Name}");
#endif
		...
		public static bool IsParamsMatch(MethodInfo delegateMethod, MethodInfo bridgeMethod)
		{
			...
			var delegateParams = GetParametersWrapper(delegateMethod);//delegateMethod.GetParameters();
			var bridgeParams = GetParametersWrapper(bridgeMethod);//bridgeMethod.GetParameters();
			if (delegateParams.Count != bridgeParams.Count)
			{
				return false;
			}
            ...
            var lastPos = delegateParams.Count - 1;
