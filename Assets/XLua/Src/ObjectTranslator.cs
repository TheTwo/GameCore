...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        Delegate getDelegate(DelegateBridgeBase bridge, Type delegateType)
        {
            ...
            var methods = bridge.GetType().GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).Where(m => !m.IsGenericMethodDefinition && (m.Name.StartsWith("__Gen_Delegate_Imp") || m.Name == "Action"));
            foreach (var m in methods)
            {
	            if (!m.IsConstructor && Utils.IsParamsMatch(delegateMethod, m))
	            {
#if !UNITY_WSA || UNITY_EDITOR
		            return Delegate.CreateDelegate(delegateType, bridge, m);
#else
        ...
        public void OutputDelegateBridge()
        {
            const int argCountLimit = 16;
            var registry = new List<int>(delegate_bridges.Count);
            foreach (var bridge in delegate_bridges)
            {
                if (bridge.Value.IsAlive)
                {
                    registry.Add(bridge.Key);
                }
            }

            for (int i = 0; i < registry.Count; i+=argCountLimit)
            {
                int oldTop = LuaAPI.lua_gettop(luaEnv.L);
                int errFunc = LuaAPI.load_error_func(luaEnv.L, luaEnv.errorFuncRef);
                LuaAPI.xlua_getglobal(luaEnv.L, "LogOutDelegateBridges");
                var args = 0;
                for (int j = 0; j < argCountLimit; j++)
                {
                    if (i + j < registry.Count)
                    {
                        LuaAPI.xlua_pushinteger(luaEnv.L, registry[i + j]);
                        args++;
                    }
                    else
                        break;
                ...
                if (args > 0)
                {
                    int error = LuaAPI.lua_pcall(luaEnv.L, args, 0, errFunc);
                    // NLogger.Error("lua_pcall:{0}", error);
                    if (error != 0)
                    {
                        luaEnv.PrintError(oldTop);
                    }
                }
            }
        }
        ...
        public void OpenLib(RealStatePtr L)
		{
            ...
            LuaAPI.lua_rawset(L, -3);
            LuaAPI.xlua_pushasciistring(L, "newtable");
            LuaAPI.lua_pushstdcallcfunction(L, StaticLuaCallbacks.NewTable);
            LuaAPI.lua_rawset(L, -3);
            LuaAPI.lua_pop(L, 1);
        ...
        //注意这个函数一定要慎用，基本只有确认断开Lua对C#对象引用不会出错才可以
        //主要引用场景是Lua中引用了一个LuaBehaviour这种方式，在LuaBehaviour中直接解除Lua的引用然后把这个LuaTable给Dispose掉
        internal void ReleaseCSObjInCS(object delete)
        {
	        var success = reverseMap.TryGetValue(delete, out var index);            
            if (success)
            {
	            //删除C#侧的缓存
	            objects.Replace(index, null);
	            reverseMap.Remove(delete);
            }
        }
