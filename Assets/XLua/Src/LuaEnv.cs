    ...
    public partial class LuaEnv : IDisposable
    {
...
#if THREAD_SAFE || HOTFIX_ENABLE
	        LockUtils.CheckLock(luaEnvLock);lock(luaEnvLock)
            {
#endif
                ...
                // AddBuildin("bin_decoder", XLua.LuaDLL.Lua.LoadBinDecoder);
        ...
        public virtual void Dispose(bool dispose)
        {
            ...
            LockUtils.CheckLock(luaEnvLock); lock(luaEnvLock)
            {
                ...
                if (!translator.AllDelegateBridgeReleased() && UnityEngine.Application.isPlaying)
                {
                    translator.OutputDelegateBridge();
                    // throw new InvalidOperationException("try to dispose a LuaEnv with C# callback!");
                    NLogger.Error("try to dispose a LuaEnv with C# callback!");
                }
        ...
        public void ThrowExceptionFromError(int oldTop)
        {
            ...
            LockUtils.CheckLock(luaEnvLock); lock(luaEnvLock)
            {
                ...
                var luaEx = new LuaStackTraceException(err.ToString());
#if UNITY_EDITOR
                // CheckLuaStackTraceException.Test(luaEx); 
#endif
                throw luaEx;
#if THREAD_SAFE || HOTFIX_ENABLE
            }
        ...
        public void PrintError(int oldTop)
        {
#if THREAD_SAFE || HOTFIX_ENABLE
            LockUtils.CheckLock(luaEnvLock); lock(luaEnvLock)
            {
        ...
        public delegate ReadOnlySpan<byte> CustomLoader2(ref string filepath);

        internal List<CustomLoader> customLoaders = new List<CustomLoader>();
        internal List<CustomLoader2> customLoaders2 = new();
        ...
        public void AddLoader(CustomLoader2 loader)
        {
	        customLoaders2.Add(loader);
        }
