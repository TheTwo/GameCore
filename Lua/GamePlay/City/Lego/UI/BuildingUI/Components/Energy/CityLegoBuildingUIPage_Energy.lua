...
---@class CityLegoBuildingUIPage_Energy:BaseUIComponent
local CityLegoBuildingUIPage_Energy = class('CityLegoBuildingUIPage_Energy', BaseUIComponent)

function CityLegoBuildingUIPage_Energy:OnCreate()
 ...
 --   self.textTime = self:Text('p_text_time')
  ...
  --  self.goVxEffectGlow = self:GameObject('vx_effect_glow')
    self.goResidentRoot = self:GameObject("p_resident_root")
    self.btnBuff2.gameObject:SetActive(false)
    self.btnBuff3.gameObject:SetActive(false)
    self.goResidentRoot:SetActive(false)
...
function CityLegoBuildingUIPage_Energy:OnFeedData(param)
...
function CityLegoBuildingUIPage_Energy:AddEventListener()
    if self.listenerAdded then return end
...
function CityLegoBuildingUIPage_Energy:RemoveListener()
    if not self.listenerAdded then return end
...
function CityLegoBuildingUIPage_Energy:OnHide()    
    self:StopTimer()
    self:RemoveListener()
end

function CityLegoBuildingUIPage_Energy:OnClose()
    self:OnHide()
end

function CityLegoBuildingUIPage_Energy:StopTimer()
    if self.timer then
...
function CityLegoBuildingUIPage_Energy:RefreshTrainingState()
...
function CityLegoBuildingUIPage_Energy:RefreshAutoLastTime()
    local endTime = ModuleRefer.TrainingSoldierModule:GetAutoTraingTime()
...
function CityLegoBuildingUIPage_Energy:RefreshCustomLastTime()
    local endTime = ModuleRefer.TrainingSoldierModule:GetCustomTraingTime()
    if endTime and endTime > 0 then
        local lastTime = endTime - g_Game.ServerTime:GetServerTimestampInSeconds()
      --  self.textTime.text = TimeFormatter.SimpleFormatTime(lastTime)
...
function CityLegoBuildingUIPage_Energy:OnEndEdit(inputText)
    if not self.isMax then
...
function CityLegoBuildingUIPage_Energy:ChangeCostResourcesNum(inputNum)
    if self.isCustomTraining then
...
function CityLegoBuildingUIPage_Energy:OnBtnDetailClicked(args)
...
function CityLegoBuildingUIPage_Energy:OnReplenishBtnClicked()
    local costItems = ModuleRefer.TrainingSoldierModule:GetCostItems(self.workId, nil, self.furnitureId, nil)
    local items = {}
...
function CityLegoBuildingUIPage_Energy:OnReplenishDisableBtnClicked()
    ModuleRefer.ToastModule:AddSimpleToast(I18N.Get("Energy_ReachedUpperLimit"))
end

function CityLegoBuildingUIPage_Energy:OnStartBtnClicked()
    local param = CastleMilitiaTrainParameter.new()
    param.args.FurnitureId = self.furnitureId
    param.args.WorkCfgId = self.workId
    --param.args.CitizenId = 0
...
function CityLegoBuildingUIPage_Energy:OnDisableBtnClicked()
...
function CityLegoBuildingUIPage_Energy:OnBtnToggleClicked(args)
    local castleMilitia = ModuleRefer.TrainingSoldierModule:GetCastleMilitia()
...
function CityLegoBuildingUIPage_Energy:OnBtnStopClicked(args)
    local castleMilitia = ModuleRefer.TrainingSoldierModule:GetCastleMilitia()
...
function CityLegoBuildingUIPage_Energy:OnBtnEfficiencyClicked(args)
    -- body
end

function CityLegoBuildingUIPage_Energy:OnBtnBuff1Clicked(args)
    -- body
end

-- function CityLegoBuildingUIPage_Energy:OnBtnBuff2Clicked(args)
--     -- body
-- end

-- function CityLegoBuildingUIPage_Energy:OnBtnBuff3Clicked(args)
--     -- body
-- end

return CityLegoBuildingUIPage_Energy