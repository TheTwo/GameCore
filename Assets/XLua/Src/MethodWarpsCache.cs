        ...
        public int Call(RealStatePtr L)
        {
            try
            {
                ...
                if (method.IsDefined(typeof(ObsoleteAttribute), true))
                {
                    ...
                    NLogger.Warn("Obsolete Method [{0}.{1}]: {2}",method.DeclaringType, method.Name, info.Message);
                } 
#endif
                object target = null;
...
#if XLUA_PROFILING
                Profiler.BeginSample($"[xLua]{method.DeclaringType}.{Name}");
#endif
    ...
    public class MethodWrap
    {
        ...
        public int Call(RealStatePtr L)
        {
            ...
            catch (System.Reflection.TargetInvocationException e)
            {
	            ...
	            var wrappedException = new LuaStackTraceException(traceback, e.InnerException ?? e);
            ...
            catch (System.Exception e)
            {
	            var t = ObjectTranslatorPool.Instance.Find(L);
	            string traceback = t.luaEnv.TraceBack();
	            var wrappedException = new LuaStackTraceException(traceback, e);
	            return LuaAPI.luaL_error_exception(L, wrappedException);
            }
        }
    }
            ...
            {
                {
                    ...
                    methodsOfType[eventName] = (L) =>
                    {
                        ...
                        try
                        {
                            object handlerDelegate = null;
                            var del = translator.FastGetCSObj(L, start_idx+2);
                            bool isDelegate = del is Delegate;
                            if (isDelegate)
                            {
                                handlerDelegate = del;
                            }
                            ...
                            {
                                handlerDelegate = translator.CreateDelegateBridge(L, eventInfo.EventHandlerType, start_idx + 2);
                            }
