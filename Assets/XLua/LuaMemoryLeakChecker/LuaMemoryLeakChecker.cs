...
namespace XLua
{
    ...
    public static class LuaMemoryLeakChecker
    {
        ...
        static Data getSizeReport(LuaEnv env)
        {
            ...
            LuaAPI.xlua_report_table_size(env.L, (IntPtr p, int size) => {
        ...
        static Dictionary<IntPtr, List<RefInfo>> getRelationship(LuaEnv env)
        {
            ...
            int top = LuaAPI.lua_gettop(env.L);
            IntPtr registryPointer = LuaAPI.xlua_registry_pointer(env.L);
            IntPtr globalPointer = LuaAPI.xlua_global_pointer(env.L);

            LuaAPI.xlua_report_object_relationship(env.L, (IntPtr parent, IntPtr child, LuaDLL.RelationshipType type, string key, double d, string key2) => {
                List<RefInfo> infos;
                try
                {
